...
class ExaminationService {
  ...
  async getExaminations(query = {}) {
    ...
    if (query.fields) {
      solution = solution.map(element => getKeys(element, query.fields));
    }
    if (query.sort) {
      if (query.sort[0] === "-") {
  ...
  async getSingleExamination(examId, query = {}) {
    let singleExamination = this.serviceExaminations.find(
      el => el.id === examId
    );
  ...
  async getExaminationsByPet(petId, query = {}) {
    ...
    if (pet) {
      ...
      if (query.fields) {
        examinationsCopy = examinationsCopy.map(el =>
          getKeys(el, query.fields)
        );
      }
